...
# ||ri,ei1, ei2|| <= tau*sqrt(2*n_Enc+18*n_F)
...
boundp= sqrt(2*n_Enc)*tau**2*sqrt(2*n_Enc+18*n_F) + tau*sqrt(2*n_Enc+18*n_F)
p = ceil(2*boundp)
print('p:', p)
...
lwe_enc_primal = MLWE_optimize_attack(q_PKE, n_Enc, n_Enc+18*n_F, tau, cost_attack=LWE_primal_cost, cost_svp=svp_classical, verbose=False)
...
# norm of [u1 & u2 & Coeff-1(x) & z1 & v01 ... v17 s] as per Thm. 5.3 (Fig. 10), in Thm.4.2 (Fig. 6)
norm_szk = sqrt( 2*(tau_x^2*n_F) + 16 * sqrt(q_F) + (beta**2) + sqrt(14 * n_F) + tau**2*(2*n_Enc+18*n_F))
#norm of the norm vector s^(e) = ( u1 | u2 | Coeff-1(x) | z1 | v01 ... v17 s | e1 | e2 | x)
alpha_e = sqrt(2*(tau_x^2*n_F) + 16 * sqrt(q_F) + (beta**2) + sqrt(14 * n_F) + tau**2*(2*n_Enc+18*n_F) + norm_x**2)
...
ce = d*(round(2*n_F/d)+1+round(2*n_F/d)+1+round((2*n_Enc+18*n_F)/d)+1) # Fig 10
...
beta4 =  16 * sqrt(q_F)                      # ||Coeff-1(x)||
beta5 = sqrt(14 * n_F)                       # ||(v01, ..., v17)||
...
q3 = 70141 #chosen as next prime to ceil(lb/q_PKE) such that it is congruent to 5 mod 8
...
cli_obt = {'d': 128, #ring dimension
              ...
              'm1': 156, # m1 = len([y1|y2|x1|x2])+len(s|e1|e2) + ve = (4+4+4+4)+(2*8+3*4)+3 = 47 (here s is the encryption secret)
...
ct_size = (n_Enc+18*n_F)*log(q_PKE,2)/(8.*Kilo)
