...
from math import lgamma, log, floor, exp, pi
from scipy.special import digamma, zeta, gammaincc, gamma
...
chi2_cdf = chi2.cdf
def chi2_CDF(n, x):
    return 1. - chi2_cdf(n,x)

#def chi2_CDF(n, x):
#    if x > 100 * n:
#        return 1.
#    return float(1. - gammaincc(n/2., x/2.)/gamma(n/2.))

chisquared_table = {i: None for i in range(2*max_n_cache+1)}
for i in range(2*max_n_cache+1):
    chisquared_table[i] = chi2(i)
#print('chisquared_table:', chisquared_table)
...
def conditional_chi_squared(d1, d2, lt, l2):
    ...
    PE2 = chisquared_table[d2].cdf(l2)#D2(l2)
    ...
    for i in range(steps)[::-1]:
        ...
        PC2 = (chisquared_table[d2].cdf(l2_max) - chisquared_table[d1].cdf(l2_min))/PE2 #(D2(l2_max) - D2(l2_min)) / PE2
        PE1 = chisquared_table[d1].cdf(lt - l2_mid)
...
def combined_attack_prob(q, n, sk_variance=2/3., ntru="circulant", fixed_tours=None, only=None, verbose=True, dsl_logvol=None):
    if n > max_n_cache:
    ...
    for beta in range(2,n):
        ...
        if remaining_proba < 0.001:
            ...
            break
    return average_beta, beta, total_SKR_prob, total_DSD_prob, prob_pos_total
