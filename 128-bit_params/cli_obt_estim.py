...
hardness = 25 # set minimal to get sufficient hardness
n_Enc = d*hardness 
...
# ||ri,ei1, ei2|| <= tau*sqrt(2*n_Enc+6*n_F)
...
boundp= sqrt(2*n_Enc)*tau**2*sqrt(2*n_Enc+6*n_F) + tau*sqrt(2*n_Enc+6*n_F)
p = ceil(2*boundp)
print('p:', p)
...
lwe_enc_primal = MLWE_optimize_attack(q_PKE, n_Enc, n_Enc+6*n_F, tau, cost_attack=LWE_primal_cost, cost_svp=svp_classical, verbose=False)
...
ve = 6 # number of norm equations
vd = 0 # number of inf-norm equations
norm_x = sqrt(ve)*sqrt(d)

# norm of [u1 & u2 & x & z1 & v0 & v1 & s] as per Thm. 5.3 (Fig. 10), in Thm.4.2 (Fig. 6)
norm_szk = sqrt( 2 * (tau_x^2*n_F) + 16 * sqrt(q_F) + (beta**2) + 2 * 16 * sqrt(q_F) + tau**2*(2*n_Enc+6*n_F))
#norm of the norm vector s^(e) = ( u1 | u2 | x | z1 | v0 | v1 | s | e1 | e2 | x)
alpha_e = sqrt(2*(tau_x^2*n_F) + 16 * sqrt(q_F) + (beta**2) + 2 * 16 * sqrt(q_F) + tau**2*(2*n_Enc+6*n_F) + norm_x**2)
...
ce = d*(round(2*n_F/d)+1+round(2*n_F/d)+1+round((2*n_Enc+6*n_F)/d)+1) # Fig 10
lb1 = floor(Be*41*ce)+1                      # Thm. 5.3
lb2 = floor(Be**2+sqrt(ve*d)*Be)+1           # Thm. 5.3
beta1 = beta                                 # ||(s1,s2)||
beta2 = sqrt(2*n_F)*tau_x                    # ||(u1,u2)||
beta3 = tau*sqrt(n_Enc+n_Enc+12*128)         # ||(s,e1,e2)||
beta4 = 16 * sqrt(q_F)                     # ||x||
beta5 = 2 * 16 * sqrt(q_F)                  # ||(v0, v1)||
lb3 = floor(Be**2+2*(max(beta1, beta2, beta3, beta4, beta5))**2)  # Thm. 5.3
lb = max(lb1, lb2, lb3)
...
q3 = 123373 #chosen as next prime to ceil(lb/q_PKE) such that it is congruent to 5 mod 8
# Maple code: q3 := 70141: d:=128: l := numelems((Factors(x^d + 1) mod q3)[2]): isprime(q1), l;
# Maple answer is "true, 2"
q_ZK = q_PKE*q3
assert(q_ZK>=lb)
assert(q3>=q1)
...
print('D:', D_candidate)
cli_obt = {'d': 128, # ring dimension
             'n': 14, # determines SIS hardness
             'm1': 69, # m1 = len([ct])+len(s|e1|e2) + ve
             'm2': 54, # m2 - n+{0,1,2}*(256/d+1)+lamb/2+ell determines LWE hardness
             'q': q_ZK, # ZK modulus
             'lambda': lamb,
...
assert(cli_obt['m2']>cli_obt['n']+scalar*(256/d+1)+cli_obt['lambda']/2+cli_obt['ell']+2)

# sizes
proof = proof_size(cli_obt)
print('proof size:', proof)
ct_size = (n_Enc+6*n_F)*log(q_PKE,2)/(8.*Kilo)
...
reps = 2*exp(14/cli_obt['gamma1'] + 1/(2*cli_obt['gamma1']**2) + 1/(2*cli_obt['gamma2']**2) + 1/(2*cli_obt['gammae']**2))
print('avg nbr of repeats:', reps)

# security
sis_sec = SIS_security(cli_obt)
print('SIS hardness:', sis_sec)
lwe_sec = LWE_security(cli_obt)
